...
export function useSurveyorForm() {
  ...
  function validate(data: FormData) {
    ...
    if (data.phoneNumber.trim() && !phoneRegex.test(data.phoneNumber.trim())) {
      e.phoneNumber =
        "Phone number must contain digits only (optional + at start).";
    }
    if (
      data.mobileNumber.trim() &&
      !phoneRegex.test(data.mobileNumber.trim())
    ) {
      e.mobileNumber =
        "Mobile number must contain digits only (optional + at start).";
    }

    if (
      (data.employmentStatus === "employee" ||
        data.employmentStatus === "owner") &&
    ...
    if (!data.qualifications.length)
      e.qualifications = "Select at least one qualification.";
    if (!data.vesselTypes.length)
      e.vesselTypes = "Select at least one vessel type.";
    ...
    const dd = Number(data.dobDD),
      mm = Number(data.dobMM),
      yy = Number(data.dobYYYY);
    if (data.dobDD && (dd < 1 || dd > 31)) e.dobDD = "Day must be 1-31.";
    if (data.dobMM && (mm < 1 || mm > 12)) e.dobMM = "Month must be 1-12.";
    if (data.dobYYYY && (String(data.dobYYYY).length !== 4 || yy < 1900))
      e.dobYYYY = "Enter a valid year.";

    const qdd = Number(data.qualifiedDD),
      qmm = Number(data.qualifiedMM),
      qyy = Number(data.qualifiedYYYY);
    if (data.qualifiedDD && (qdd < 1 || qdd > 31))
      e.qualifiedDD = "Day must be 1-31.";
    if (data.qualifiedMM && (qmm < 1 || qmm > 12))
      e.qualifiedMM = "Month must be 1-12.";
    if (
      data.qualifiedYYYY &&
      (String(data.qualifiedYYYY).length !== 4 || qyy < 1900)
    )
      e.qualifiedYYYY = "Enter a valid year.";
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (!r?.name?.trim())
        e[`ref_${i}_name`] = `Reference ${i + 1} name is required.`;
      if (!r?.contact?.trim())
        e[`ref_${i}_contact`] = `Reference ${i + 1} contact is required.`;
    }
  ...
  function handleChange(
    ...
    setFormData((prev) => {
      ...
      if (name === "employmentStatus" && value === "self")
        next.companyName = "";
      return next;
    });
  ...
  function handleExperienceChange(
    ...
    // limit length
    if (field === "years")
      cleaned = cleaned.slice(0, 2); // 0-99 years
  ...
  function handlePhotoChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file.type.startsWith("image/")) {
      ...
      setErrors((prev) => ({
        ...prev,
        photoFile: "Please upload an image (JPG/PNG/WebP).",
      }));
      return;
    }
    ...
    if (file.size > MAX) {
      ...
      setErrors((prev) => ({
        ...prev,
        photoFile: "Photo must be 20MB or less.",
      }));
      return;
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      payload.append(
        "experienceByQualification",
        JSON.stringify(formData.experienceByQualification),
      );
      ...
      payload.append(
        "shoresideExperience",
        JSON.stringify(formData.shoresideExperience),
      );
      payload.append(
        "surveyingExperience",
        JSON.stringify(formData.surveyingExperience),
      );
      payload.append(
        "vesselTypeSurveyingExperience",
        JSON.stringify(formData.vesselTypeSurveyingExperience),
      );
      payload.append("accreditations", JSON.stringify(formData.accreditations));
      payload.append(
        "coursesCompleted",
        JSON.stringify(formData.coursesCompleted),
      );
